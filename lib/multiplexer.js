...
util.extend(Multiplexer.prototype, {
    ...
    //     //here we must detect if query is a read query (select) or anything else
    //     if (!Multiplexer._replication) {
    ...
    //
    //         // return redeffer(self._model.query(sql, params), self, true);
    //         })
    //     }
    //
    //     self._connect(function (error, threadId) {
    ...
    //
    //         // call function of underlying model
    //         sails.log.debug("oot, kas me kasutame nüüd readonly't");
    //         self._model.query(sql, params, function (err, result) {
    //             self._disconnect(threadId);
    //             callback(err, result); // no need to wrap query since result is an integer
    //         }, threadId);
    //     });
